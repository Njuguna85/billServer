...
router.get("/datalytics", async (req, res) => {
  ...
  try {
    const responseToken = await axios.post(
      "https://bi.datalytics.co.ke/oauth/token",
      {
    ...
    const responseBillboards = await axios.get(
      "https://bi.datalytics.co.ke/api/billboard/",
      {
        headers: {
    ...
    const responseDeliveries = await axios.get(
      "https://bi.datalytics.co.ke/api/all-deliveries",
      {
        headers: {
          Authorization: `Bearer ${responseToken.data.access_token}`,
          "Access-Control-Allow-Methods": "GET",
    ...
    `SELECT 
        ...
        where 
            ST_Contains(st_setsrid(ug.geom, 4326),
                        st_geomfromtext('POINT(${req.params.long} ${req.params.lat})', 4326));`,
    { type: sequelize.QueryTypes.SELECT }
  );
